...
def run_introduction_section():
    ...
    print("The area is", square, "sqft.")
    print(
        "\nFinally a person wants to take out $150 from an atm that dispenses "
        "only $20 bills.")
    money_dispensed_guess = int(
        input("How much money will they be able to take out? "))
...
def attach_list_of_purchases(list_of_purchases):  # This function accepts a
    # list parameter and prints each index of the list.
...
def calculate_shopping_trip():
    ...
    print("I'm here to help make sure you don't overspend on your"
          " shopping spree.")
    spending_limit = int(input(
        "What's your spending limit for this shopping trip? "
        "\nEnter it here: "))
    ...
    while spending_limit >= 5 and not exit_loop:  # Two factors to continue
        ...
        # false.
        first_item = float(
            input("How much is the item you are going to purchase"
        ...
        new_price = 0
        if item_on_sale == "yes":  # If item is equal to yes it will go into
            # the if statement
        ...
        if answer == "no":
            exit_loop = True  # This makes the the user exit the loop by making
            # it true instead of false.
